...
//  2) create a public setter using the inherited onlyOwner modifier 
    function setPaused(bool inp) onlyOwner() public returns (bool state){
        _paused = inp;
        if (inp)
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require (owner != to, "owner cannot send token to himself");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == getOwner() || isApprovedForAll(owner, msg.sender), "message sender is not owner or is not approved for all"); 
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(ownerOf(tokenId) == from, "token is not owned by address!");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function _setTokenURI(uint256 tokenId) internal {
        ...
        string memory tokstr = usingOraclize.uint2str(tokenId);
        _tokenURIs[tokenId] = usingOraclize.strConcat(_baseTokenURI, tokstr);
    }
...
contract ERC721Mintable is ERC721Metadata {
    constructor(string memory _name, string memory _sym, string memory _BTURI)  ERC721Metadata(_name, _sym, _BTURI) public {
